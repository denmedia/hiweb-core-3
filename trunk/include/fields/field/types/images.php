<?php
	...
	namespace {
		...
		if( !function_exists( 'add_field_images' ) ){
			/**
			 * @param $id
			 * @return \hiweb\fields\types\images
			 */
			function add_field_images( $id ){
				$new_field = new hiweb\fields\types\images( $id );
	...
	namespace hiweb\fields\types {
		...
		class images extends field{
			...
			public function admin_get_input( $value = null, $attributes = [] ){
				...
				ob_start();
				?>
				<div class="postbox hiweb-field-images" <?= $this->admin_get_input_attributes_html() ?>>
						...
						<div class="items">
							<?php
								if( $this->have_images( $value ) ){
									foreach( $this->value_sanitize($value) as $item ){
										$this->html_row( $item );
									}
								}
							?>
